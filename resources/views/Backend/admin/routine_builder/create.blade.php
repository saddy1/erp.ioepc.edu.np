...
@section('content')
    <div class="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <form method="GET" action="{{ route('routine.builder.create') }}"
            class="mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
            <div>
            ...
            @if ($exam)
                ...
                <div>
                    ...
                    <input type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-50"
                        value="{{ convertTo12Hour($exam->start_time) }}" readonly>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">End Time</label>
                    <input type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-50"
                        value="{{ convertTo12Hour($exam->end_time) }}" readonly>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Batch</label>
                    <input type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-50"
                        value="{{ $exam->batch == 'new' ? 'New (1)' : 'Old (2)' }}" readonly>
                </div>
            @endif
        </form>
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <select name="semester"
                        ...
                        @foreach ($allowedSems as $s)
                            <option value="{{ $s }}" @selected($selectedSemester == $s)>Semester {{ $s }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <button
                    class="border border-gray-300 rounded-lg px-5 py-2 text-sm bg-white hover:bg-gray-50 font-medium transition">Load</button>
            </form>
        @endif
            ...
            @php
                ...
                foreach ($slots as $slot) {
                    ...
                    foreach ($slot->subjects as $sub) {
                        ...
                        // Get subject name from FacultySemesterSubject -> subject (master table)
                        $subjectDetail = \App\Models\FacultySemesterSubject::with('subject')
                        ...
                        $subjectName = $subjectDetail && $subjectDetail->subject ? $subjectDetail->subject->name : '';
                ...
                foreach ($matrix as $date => $facData) {
                    foreach ($facData as $subData) {
                        if (!in_array($subData['code'], $allSubjects)) {
                            $allSubjects[] = $subData['code'];
                        }
                    }
                }
                $colors = [
                    ...
                    <table class="min-w-full text-xs">
                        <thead>
                            <tr class="bg-gray-50 border-b">
                                <th
                                    class="px-3 py-2 text-left font-semibold text-gray-700 sticky left-0 bg-gray-50 z-10 min-w-[120px]">
                                    <div>Date (BS)</div>
                                    <div class="text-[10px] font-normal text-gray-500">Time</div>
                                </th>
                                @foreach ($faculties as $f)
                                    <th
                                        class="px-2 py-2 text-left font-semibold text-gray-700 whitespace-nowrap min-w-[100px]">
                                        {{ $f->code }}
                                    </th>
                                @endforeach
                            </tr>
                        ...
                        <tbody>
                                ...
                                <tr class="border-b hover:bg-gray-50 transition">
                                    ...
                                    @foreach ($faculties as $f)
                                        <td class="px-2 py-2">
                                            ...
                                            @if ($subData)
                                                <div
                                                    class="inline-block rounded-md px-2 py-1 {{ $subjectColors[$subData['code']] ?? 'bg-gray-100 text-gray-800' }}">
                                                    <div class="font-semibold text-xs">{{ $subData['code'] }}</div>
                                                    @if ($subData)
                                                        <div class="text-[9px] leading-tight mt-0.5 opacity-90">
                                                            {{ $subData['name'] }}</div>
                                                    @endif
                                                </div>
                                            @else
                            ...
                            @empty
                                <tr>
                                    <td class="px-4 py-6 text-center text-gray-500 text-sm"
                                        colspan="{{ 1 + $faculties->count() }}">
                ...
                <form method="POST" action="{{ route('routine.builder.store') }}">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button type="button" id="addDayBtn"
                            class="border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium hover:bg-gray-50 transition">
                            + Add Slot (Suggest +3 BS days)
                        </button>
                        <button type="submit"
                            class="bg-gray-900 text-white rounded-xl px-5 py-2.5 text-sm font-semibold hover:bg-gray-800 transition shadow-sm">
                            Save Routine
                        </button>
                    </div>
                </form>
    ...
    @if ($exam && $selectedSemester)
        <script>
            ...
            function monthDaysBs(y, m) {
                return BS_MONTH_DAYS_DEFAULT[m - 1] || 30;
            }

            function addDaysBs(str, n) {
                let p = parseBs(str);
                if (!p) return '';
                while (n-- > 0) {
                    const md = monthDaysBs(p.y, p.m);
                    if (p.d < md) p.d++;
                    else {
                        p.d = 1;
                        p.m === 12 ? (p.m = 1, p.y++) : p.m++;
                    }
                }
                return formatBs(p);
            }
